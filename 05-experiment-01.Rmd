...
In [Experiment 1](#experiment1a), participants were instructed to only repeat the final word of the sentence they heard,
while in [Experiment 2](#experiment1b), they were instructed to repeat the whole sentence, and by this drawing attention to the entire sentence including the context.
...
As we have discussed earlier in [Chapter 1](#chapter-introduction) and [2](#chapter-background), it is generally agreed upon that human language processing is predictive in nature and comprehenders generate expectations about upcoming linguistic materials based on the context available to them [for reviews, see @Kuperberg2016; @Nieuwland2019; @Pickering2018; @Staub2015].
...
We varied the task instructions to the listeners from [Experiment 1](#experiment1a) to [Experiment 2](#experiment1b) which required them to differentially attend to the target word (not binding the context), or to the target word including the context.
...
In contrast, when listeners attend to the whole sentence, they do form expectations such that the facilitatory effect of target word predictability will be observed replicating the prior behavioral findings [e.g., @Obleser2010].

## Experiment 1 {#experiment1a}
...
Accuracy was analyzed using Generalized Linear Mixed Models (GLMMs) following the procedure described in Chapter \@ref(chapter-stats) (Section \@ref(analysis-main)) with lmerTest [@Kuznetsova2017] and lme4 [@Bates2015] packages.
...
Based on the previous findings on perceptual adaptation [e.g., @Cooke2022; @Davis2005; @Erb2013], we further added trial number (centered) in the fixed effect structure to control for whether the listeners adapted to the degraded speech.
...
```{r figure1a, eval=TRUE, echo=FALSE, fig.align='center', fig.cap="Mean response accuracy across all conditions in Experiment 1. Accuracy increased only with an increase in the number of noise-vocoding channels. There is no change in accuracy with an increase or decrease in target word predictability. Error bars represent standard error of the means.", out.width="95%"}
...
and 6-channel vocoded speech ($\beta$ = -.70, SE = .21, *z*(4,410) = -3.29, *p* = .001),
...
```{r figure1b, eval=TRUE, echo=FALSE, fig.align='center', fig.cap="Mean response accuracy across all conditions in Experiment 2. Accuracy increased with an increase in number of noise-vocoding channels and target word predictability. Error bars represent standard error of the means.", out.width="95%"}
...
in the 8-channel ($\beta$ = 1.42, SE = .62, *z*(1,440) = 2.30, *p* = 0.02)
and 6-channel noise-vocoding conditions ($\beta$ = 1.14, SE = 0.34, *z*(1,440) = 3.31, *p* < .001);
...
($\beta$ = .04, SE = .26, *z*(8,730) = .15, *p* = .88) indicating that the overall response accuracy did not change with the change in instructions from Experiments 1 and 2.
...
Obleser and colleagues reported that at  at 8-channel noise-vocoded speech, key word recognition[^measurement-difference] was higher in high predictability sentences than low predictability sentences.
...
When listeners' attention is focused on one feature of an auditory stimuli, the rest are not attended to and they are not accessed [filter mechanism, @Lange2013; change detection, @Sanford2006; @Sturt2004]
...
It is not implausible that the listeners listened to the context but did not do a deep processing [cf. @Ferreira2016].
...
Hence, we find some discrepance between the results of prior studies [@Obleser2007; @Obleser2010] and our study (Experiment 1).
...
<!-- We use a metric of measuring language comprehension that considers listeners' use of context information.  -->

[^measurement-difference]: Notice the difference in measurement metric. They calculated response accuracy as the number of correct keywords identified, while we calculated it as the correct identification of the sentence-final target word.