...
One of them is an adverse listening condition.
...
Applying visual world paradigm, studies have demonstrated that individuals show anticipatory eye movements towards a picture of an object (e.g., \emph{a cake}) that is predictable from the preceding sentence context (e.g., \emph{The boy will eat \ldots{}}) even before hearing the final target word (\protect\hyperlink{ref-Altmann1999}{Altmann \& Kamide, 1999}; \protect\hyperlink{ref-Ankener2018}{Ankener et al., 2018}; \protect\hyperlink{ref-Kamide2003}{Kamide et al., 2003}).
...
it follows that comprehenders make predictions at a slow presentation rate, but not at a fast presentation rate.
...
We propose that Pickering \& Gambi (\protect\hyperlink{ref-Pickering2018}{2018})'s prediction-by-production account can be enriched by incorporating the instances of varying predictability effects at fast and slow speech rate in degraded speech comprehension.
...
Additionally, word-form predictions --- that the listeners make in the experiments reported in this thesis --- are shown to be inefficient in facilitating adaptation to degraded speech (\protect\hyperlink{ref-Corps2020}{Corps \& Rabagliati, 2020}).
...
In contrast to prior studies, we show that listeners do not adapt to degraded when despite a repeated exposure to the same degraded speech
...
These random effects are assumed to be random samples drawn from the general population (\protect\hyperlink{ref-Baayen2008}{Baayen et al., 2008}).
...
As mentioned above, simple regression model including, ANOVA, does not model the effect of two random effects simultaneously which a mixed effects model does.
...
Averaging further eliminates the variability in the data.
Additionally, comparing the means of a categorical variable (correct vs.~incorrect responses) even when transformed into accuracy or proportion scale is hard to interpret in a sensibly as compared to a continuous variable like reaction time (for discussion see \protect\hyperlink{ref-Jaeger2008}{Jaeger, 2008}).
The statistical remedy for these both problems is to apply mixed effects models to our data.
...
Although the design itself is balanced, after removal of outliers and/or trials which are not appropriate for comprehension measures,
...
For such reasons, mixed models are used and advocated for by psycholingustis and statisticians (\protect\hyperlink{ref-Barr2013}{Barr et al., 2013}; \protect\hyperlink{ref-Meteyard2020}{Meteyard \& Davies, 2020}; \protect\hyperlink{ref-Gries2015}{Th. Gries, 2015}).
...
In our data, the output variable is binary: correct or incorrect response.
As the name suggests, the output variable \(y\) in the binomial \emph{logistic} mixed effects model is in the \emph{logit} scale; \(\log\) with base \(e\), i.e.~\(\ln\).
The model therefore predicts logits of the outcome variable.
The output variables are truncated in both \emph{probability} scale and in \emph{odds} scale which range from 0 to 1 and from 0 to \(-\infty\) respectively
Therefore, in a binomial logistic regression model, log-odds (or \emph{logits}) are used which range from \(-\infty\) to \(+\infty\), and do not violate the model assumptions regarding the range of outcome variable.
...
\begin{align} \label{eq:logit-to-prob_short}
...
Equations @ref(eq:logit-to-prob\_short), and \eqref{eq:logiteq} provide the relationship between probability, logits (or log-odds), and odds (\(\frac{p}{1-p}\)).
...
The best fitting model, selected based on Akaike Information Criterion (\protect\hyperlink{ref-Grueber2011}{Grueber et al., 2011}; \protect\hyperlink{ref-Richards2011}{Richards et al., 2011}), among the parsimonious and extended models was then selected as the best fitting model for our data.
...
Contrast coding of each factors and the model description are presented in the chapters that follow.

\hypertarget{summary-1}{%
\section{Summary}\label{summary-1}}

In this chapter, we introduced the statistical tool used in the data analyses in this thesis.
We discussed the limitations in traditional linear regression based models including ANOVA.
To capture the variability of our data without averaging it out across participant or item,
and to account for the effect of two random effects --- participant and item --- simultaneously,
we fitted mixed effects models to our data.\\
Details of each dataset corresponding to each experiment is presented in the chapters that follow.
...
Results indicated better word recognition in high predictability sentences at moderate levels of degradation only when attention was directed to the whole sentence including the context information.
...
To utilize the context information in a sentence, however, listeners must attend to it and build up a meaning representation of what has been said.
...
Auditory attention allows a listener to focus on the speech signal of interest (for reviews, \protect\hyperlink{ref-Fritz2007}{Fritz et al., 2007}; cf. \protect\hyperlink{ref-Lange2013}{Lange, 2013}).
...
Hence, this small change in task instructions from Experiment 1A to Experiment 1B sheds light on the role of top-down regulation of attention on using context for language comprehension in a noisy channel.
...
more weight on the prior \(P(u_i,m_i)\) derived from the context information that facilitates langauge comprehension.
Examining the modulatory role of attention,
we saw in this chapter that the facilitatory effect is observed only when the listeners attend to the entire sentence context.
For equivalent comparison of results across two experiments,
...
We also further examine the granularity of predictability effect, and examine the perceptual adaptation to degraded speech.
