...
In this thesis, we investigated how listeners can use context information and form predictions while listening to a speech at different levels of degradation.
...
Taken together, this thesis contributes to the nuanced understanding of the dynamic interaction between top-down and bottom-up processes in degraded speech comprehension.
\end{abstract}
...
This weighing of top-down and bottom-up processes is considered as a rational process by the models of probabilistic language processing in reading comprehension (\protect\hyperlink{ref-Levy2008}{Levy, 2008}; \protect\hyperlink{ref-Ryskin2018}{Ryskin et al., 2018}; see also \protect\hyperlink{ref-vanOs2021}{van Os et al., 2021}).
...
The general limitations of the studies are also discussed.
...
The noisy channel model of communication (see Chapter \ref{chapter-introduction} for elaborate discussion) is revisited and discussed in relation to the findings of the our studies.
We also discuss the implications, limitations, and the outlook and considerations for the future research.
...
In Chapter \ref{chapter-attention-prediction}, we showed that prediction does not always occur in language comprehension.
...
These results showed that attention (or lack thereof) can limit predictability effect in degraded speech comprehension: \emph{no attention, no prediction}.
...
Since we already showed in Chapter \ref{chapter-attention-prediction} that attention to the context is necessary for predictability effect,
...
At the extremes, listeners did either not utilize the context (unintelligible at 1-channel),
or, the context and target word were clearly intelligible ---
in the latter case, listeners identified the target word based on the bottom-up information rather than only the context.
...
In two experiments, we performed the manipulation of the bottom-up processes by changing the speech rates:
...
The findings of the experiments presented in this chapter showed that slow speech does not amplify the contextual facilitation initially observed in normal speech rate.
...
Findings from Chapters \ref{chapter-attention-prediction} and \ref{chapter-graded-prediction} also showed that regardless of (un)certainty about the quality of subsequent trials,
...
In Chapters 6 and 7, we argued for a new approach to calculate response accuracy that takes into account a listener's context identification instead of other word recognition scores (e.g., proportion of correctly identified words per sentence)
...
Taken together, the contribution of this thesis can be stated as:
...
Such a contextual facilitation is optimal at a normal speech rate, which is not necessarily amplified by slowing down the speech.
However, increasng the speech rate reduces the contextual facilitation by restricting the lexical access in the less predictable sentence endings.
\end{quote}
...
The results of the experiments presented in this thesis as outlined above suggest that listeners are \emph{rational} comprehenders:
...
Regardless of what their predictions from the context are
their comprehension is a result of what they actually perceive (e.g., at 8-channel noise-vocoded speech in Chapter \ref{chapter-graded-prediction}).
...
Instead, our findings are in line with the probabilistic accounts of predictive language processing (\protect\hyperlink{ref-Delong2005}{DeLong et al., 2005}; \protect\hyperlink{ref-Kuperberg2016}{Kuperberg \& Jaeger, 2016}; cf. \protect\hyperlink{ref-Nieuwland2018}{Nieuwland et al., 2018}) proposed in the literature of speech perception and reading studies.
To our knowledge, the empirical evidence presented in this thesis is the only one since Strauß et al. (\protect\hyperlink{ref-Strauss2013}{2013}) to examine the nature of predictability effect in degraded speech comprehension.

\hypertarget{attention-and-prediction}{%
\subsection{Attention and prediction}\label{attention-and-prediction}}

Our findings reported in Chapter \ref{chapter-attention-prediction} showed that attention and prediction can operate independently (i.e., they are two separate processes) in predictive processing of degraded speech (see also \protect\hyperlink{ref-Astheimer2011}{Astheimer \& Sanders, 2011}; \protect\hyperlink{ref-Li2017}{X. Li et al., 2017}).
...
Only attention to the context provided a contextual facilitation.
This role of attention is not fully addressed in the current frameworks of predictive language processing.

In their \emph{good enough processing} framework, Ferreira \& Lowder (\protect\hyperlink{ref-Ferreira2016}{2016}) speculated that a comprehender can focus and process one part of sentence ``deeper'' and other parts can be ignored or processed at a ``shallow'' level.
However, their proposal is not clear on how attention to different parts of a speech stream can be strategically allocated,
and how it in turn moderates contextual facilitation.
...
It is supposed that listeners ideally have an access to all the stages of lexical processing.
But their account of does not include the role of attention at any stage of lexical processing in speech production, comprehension, and prediction.
...
Therefore, an elaborate and explanatory theory of predictive language processing should also account for attention regulation that modulates the effects of prediction in language comprehension.

\hypertarget{speech-rate}{%
\subsection{Speech rate}\label{speech-rate}}

We showed that sentence context facilitates comprehension of degraded speech at the normal speech rate.
Contrary to common wisdom and previous findings (e.g., \protect\hyperlink{ref-Dambacher2012}{Dambacher et al., 2012}; \protect\hyperlink{ref-Wlotko2015}{Wlotko \& Federmeier, 2015}), slowing down the speech rate did not improve contextual facilitation (cf. \protect\hyperlink{ref-Winn2021}{M. B. Winn \& Teece, 2021}).
Our results from younger adults did not show any benefit of slow speech either.
...
The speech that the people with sensory neural hearing loss and cochlear implants perceive is spectrally degraded (\protect\hyperlink{ref-Parida2022}{Parida \& Heinz, 2022}; \protect\hyperlink{ref-Shannon1998}{Shannon et al., 1998}; \protect\hyperlink{ref-Shannon2004}{Shannon et al., 2004}).
The findings from our study can inform the clinical and rehabilitative set up:
We don't find an evidence that speaking slow benefits degraded speech comprehension,
while we provide an evidence that speaking fast is detrimental to processing sentences that are not easily predictable from the context.
Therefore, in a rehabilitative set up, auditory-verbal training to the cochlear implantees can likely benefit with normal speech rate,
than the exaggerated slowing down of the speech.
...
For example, Pickering \& Gambi (\protect\hyperlink{ref-Pickering2018}{2018})'s account of predictive language processing posits that comprehenders have an access to word-form at the late stage of lexical processing.
Our findings indicate that at a fast speech rate, when the speech is degraded, processing of the less predictable words do not reach the late stage in contrast to the highly predictable words.
...
However, it is also likely that another group of 50 participant would respond ``kite'' as the most likely ending of the same sentence.
This example illustrates that a cloze-based measure is an inconsistent estimate of predictability.

In the recent years, alternatives to the cloze-based measures have been proposed.
...
Similarly, Hofmann et al. (\protect\hyperlink{ref-Hofmann2021}{2021}) demonstrated that surprisal-based measures estimated from Large Language Models (e.g., GPT-2, GPT-3, etc.) explain N400 effects and reading times better than cloze-based measures (see \protect\hyperlink{ref-Heilbron2022}{Heilbron et al., 2022} for an implementation of surprisal calculated from GPT-2).
As the growing number of studies show that the values other than cloze probability are better estimates of predictability,
it is not an overstatement to say that these measures can replace cloze probability that tends to be inconsistent.
...
For example, working memory capacity, processing speed, literacy, and language experience vary across participants and these factors result in the differences among individuals' language processing and the use lexical-semantic cues (\protect\hyperlink{ref-Federmeier2010}{K. D. Federmeier et al., 2010}; \protect\hyperlink{ref-Mishra2012}{Mishra et al., 2012}; \protect\hyperlink{ref-Rommers2015}{Rommers et al., 2015}; \protect\hyperlink{ref-Scholman2020}{Scholman et al., 2020}).
However, the general conclusions about the top-down--bottom-up interactions presented in this thesis are based on the mean estimates of predictability (in terms of the response accuracy) across all participants.
...
One of the goals of the thesis was to replicate the predictability effects in degraded speech comprehension as showed in Obleser et al. (\protect\hyperlink{ref-Obleser2007}{2007}) and Obleser \& Kotz (\protect\hyperlink{ref-Obleser2010}{2010}).
Therefore, we created short Subject--Verb--Object sentences which were similar to Obleser \& Kotz (\protect\hyperlink{ref-Obleser2010}{2010})'s stimuli.
In these sentences the verb was predictive of the noun.
However, in daily conversations, the utterances a speaker produces are not structured in this short format in which only one preceding word provides a context to predict a succeeding word.
...
Similarly, the speaker indicates important words or concepts via pitch contours, stress, or intonation patters,
which then direct the attention of a listener.
...
\hypertarget{the-nature-of-predictability-effect}{%
\subsection{The nature of predictability effect}\label{the-nature-of-predictability-effect}}

Strauß et al. (\protect\hyperlink{ref-Strauss2013}{2013}) formulated their ``narrowed expectations'' framework based on the results of an EEG study measuring the latency and amplitude of the N400 component.
We did not find a support for their claims;
...
We note that the EEG experiment proposed here was planned to be conducted as a part of this thesis,
but due to the closure of the electrophysiology lab during the covid-19 lockdown, the experiment was not conducted.
...
to investigate whether the nature of predictability effect is graded,
to examine if attention and speech rate limit or moderate contextual facilitation,
and if listeners' adaptation to degraded speech is influenced by lexical-semantic properties of the speech.
...
It was also revealed that an increase in speech rate is detrimental to the processing of low predictability sentence
We argued for the use of a metric of language comprehension measure that takes into account whether listeners' identification of context information.

Despite these findings, this thesis does not answer all the questions about predictive language processing, especially about prediction in adverse listening condition.
...
We hold a similar position with regards to our evidence on the limitations of contextual facilitation (in Chapter \ref{chapter-attention-prediction}):
...
We discussed the limitations of the experiments presented in this thesis and of the widely used methods and metrics (e.g., cloze probability, key word recognition accuracy)
...
Building on the findings of this thesis, and addressing these recommendations in the future research,
