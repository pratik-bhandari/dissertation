...
These random effects are assumed to be random samples drawn from the general population [@Baayen2008].
In this thesis, the dependent variable (or an *outcome*, or a *response* variable) is binary (correct vs incorrect response).
...
In linear regression, a dependent variable (or an *outcome*) is modeled as a function of one or more independent predictor variables (or *factors*, or *explanatory* variables).
...
Analysis of Variance (ANOVA) which is also a form of linear regression [@Chatterjee2012; @Vasishth2022] compares the means and variances of two or more conditions.
...
These problems of using ANOVA in language sciences have been pointed out since as early as the 1960s [@Clark1973; @Coleman1964].  

<!-- ### Limitations of linear regression -->

#### Modeling two random effects simultaneously, and Variability in the data

As mentioned above, simple regression model including, ANOVA, does not model the effect of two random effects<!-- --- participants and items --- --> simultaneously which a mixed effects model does.
In traditional ANOVA approach, researchers often resort into running two separate regression models [@Lorch1990] by averaging raw data across participants, and items.
<!-- This further complicates the results. -->
Averaging further eliminates the variability in the data.
Additionally, comparing the means of a categorical variable (correct vs. incorrect responses) even when transformed into accuracy or proportion scale is hard to interpret in a sensibly as compared to a continuous variable like reaction time [for discussion see @Jaeger2008].
...
This introduces a bias in the regression model [@Jaeger2008].
...
As the name suggests, the output variable $y$ in the binomial *logistic* mixed effects model is in the *logit* scale; $\log$ with base $e$, i.e. $\ln$.
The model therefore predicts logits of the outcome variable.
The output variables are truncated in both *probability* scale and in *odds* scale which range from 0 to 1 and from 0 to $-\infty$ respectively
Therefore, in a binomial logistic regression model, log-odds (or *logits*) are used which range from $-\infty$ to $+\infty$, and do not violate the model assumptions regarding the range of outcome variable.
...
Log-odds of correct response obtained from Equation \@ref(eq:mixed-effects2) can also be transformed into probability of correct response.
Equations \@ref(eq:logit-to-prob_short), and \@ref(eq:logiteq) provide the relationship between probability, logits (or log-odds), and odds ($\frac{p}{1-p}$).  
...
The model is fitted with a maximal random effects structure that included random intercepts for each participant and item [@Barr2013].
...
The best fitting model, selected based on Akaike Information Criterion [@Grueber2011; @Richards2011], among the parsimonious and extended models was then selected as the best fitting model for our data.  
...
{1 + exp (\alpha + u_{\alpha} + w_{\alpha} +
                      (\beta_{1} + u_{\beta_{1}} + w_{\beta_{1}})\cdot {x_1} + 
                      (\beta_{2} + u_{\beta_{2}} + w_{\beta_{2}})\cdot {x_2} + 
                      ... + (\beta_{n} + u_{\beta_{n}} + w_{\beta_{n}})\cdot {x_n})}}\\ \label{eq:logit-to-prob_short}
&= {\frac{exp(\ln(\frac{p}{1-p}))}{1 + exp (\ln(\frac{p}{1-p}))}}
\end{align}
-->
