...
One of the features that distinguishes us humans from other species is our ability to communicate using verbal language [@Lieberman2013]. We speak. We listen. We understand. This seemingly straight forward path of communication goes through plenty of hindrances. One of them is adverse listening condition. Human comprehenders rely on top-down predictive processes and bottom-up auditory processes to understand spoken language. Language comprehension in adverse listening conditions is aptly described and represented by the noisy channel model of communication [@Shannon1948; @Levy2008; @Gibson2013; @Gibson2019, see Figure \ref{fig:noisy-channel}, Equation \ref{eq:noisy-channel}] based on which we study the interaction between the top-down and bottom-up processes involved.
...
The receiver (listener) perceives the signal as $u_p$ and decodes it to recover the meaning as $m_p$.
<!-- For a successful communication to take place, the message recovered by the listener must be approximately equal to the message sent by the speaker. --> Human language comprehension system is assumed to be engaged in an optimal Bayesian decoding that uses all the sources of information (e.g., prior probability of plausible utterances, linguistic and world knowledge, context information) and infers the intended meaning of the speaker from the perceived meaning that it receives from a noisy channel of communication [e.g., @Levy2008; @Gibson2013].
Given the perceived utterance $u_p$, the listeners assigns a probability, $P(u_i|u_p)$, to a the intended utterance $u_i$ which is not directly available to the listener.
Using Bayes' rule, $P(u_i|u_p)$ can be re-written as:

```{=tex}
\begin{align} \label{eq:noisy-channel}
P(u_i|u_p) \propto P(u_i) P_N(u_p|u_i)
\end{align}
```
In Equation \ref{eq:noisy-channel}, $P(u_i)$ is the prior probability that a speaker intends to utter $u_i$.
...
The channel of transmission can become a noisy channel due to factors like background noise present in a conversation;
poor signal transmission in a phone call that distorts the speakers speech;
hearing loss of a listener;
hearing aid or cochlear implant wore by a listener; and so on.
To understand speech in such a noisy channel of communication, a listener puts different weights to the distorted bottom-up auditory input $P(u_p|u_i)$ vs her top-down information $P(u_i)$ (e.g., knowledge about the world, context information, etc.).
...
Most readers would expect the final word to be *a kite* in this sentence [@Delong2005; cf. @Nieuwland2020a]. Here, the words up to the final word of the sentence provide a context: A reader can utilize their knowledge about what a *boy* would ideally do *outside* when a day is *breezy*. It leads the reader to predict that the sentence continuation is most likely *a kite* and not any other word like *a rocket* . Similar results are observed in auditory domain as well. Listeners use context information from what they have heard, and form predictions about upcoming word [e.g., @Altmann2007; @Ankener2019].
When the speech signal is degraded, i.e., in a noisy channel, listeners put more weight on the prior knowledge and context information than on the auditory information [@Sheldon2008a; @Obleser2007].
They rely more on predictive processing.

Inspired by the noisy channel model of communication, this thesis investigates the interaction between top-down predictive and bottom-up auditory processes.
We investigate what the optimal level of noise in the signal is for the effect of top-down predictive processes to be most efficient or facilitatory to understand speech in a noisy channel.
Similarly, by manipulating different factors of top-down as well as bottom-up processes (e.g., attention allocation to parts of speech stream, different speech rate),
we examine the role that they play in aiding or interfering comprehension in a noisy channel.
While doing so, we address the following challenges and research goals:
