...
In Experiment [2](#experiment1b) of Chapter \@ref(chapter-attention-prediction), there was an implicit assumption that all the noun-correct responses were borne out of correctly identifying the context-evoking words (i.e., verbs).
...
In the literature on speech perception and sentence processing, studies have argued that prediction is either probabilistic and graded or all-or-nothing [e.g., @Coltheart2004; @Kuperberg2016; @Luke2016].
...
@Erb2013 and @Hakonen2017 used *report scores* [@Peelle2013] that measure the proportion of correctly recognized *words per sentence* as an index of language comprehension.
...
Using of three levels of target word predictability (low, medium and high) will let us test the narrowed expectations view [@Strauss2013] by also taking into account the accuracy of context.
...
To rule out this possible explanation of our findings, we conducted an additional analysis comparing the *forward predictability effect* (from verb to noun) to the size of the *backward predictability effect* (correct identification of the noun based on the final verb).
...
In such a paradigm, one could use the word recognition score without considering the context recognition [e.g., @vanOs2021].
However, such a paradigm introduces a confound:
linguistic materials are processed differently in the brain when presented auditorily vs visually [e.g., @Robinson2018],
and listeners do not necessarily identify the visually presented context [e.g., visual half-field recognition superiority, @McKeever1977]
