...
Although many studies support the general idea of Strauß and colleagues that predictability facilitates comprehension of degraded speech [e.g., @Sheldon2008a; @Obleser2010; @Hunter2018],
...
that some studies [@Sheldon2008a; @Obleser2007; @Obleser2010; see also, @Hunter2018] have already shown the facilitatory effect of predictability in comprehension of degraded speech.
...
They proposed that an adaptive control system (cingulo-opercular circuit) might be involved to optimize task performance when listeners are uncertain about an upcoming trial [@Vaden2015; @Vaden2016; @Eckert2016].
...
Some studies that reported contextual facilitation in degraded speech comprehension used proportion of correctly reported *final* words only [e.g., @Sheldon2008a; @Obleser2010; @Hunter2018].
...
and more importantly, none of these measures take into account if listeners have correctly identified the context [cf. @Amichetti2018; see also, @Marrufo2019]
...
We created *unpredictable channel context* group from participants (n = 48; $\bar{x}$  $\pm$ SD = 24.44 $\pm$ 3.5 years; age range = 18 -- 31 years; 16 females) from <!--[Experiment 1B](\@ref(experiment1b)).-->
...
We used the same stimuli described in [Chapter 3](#experimental-materials).
...
Following the procedure described in [Chapter 4](#analysis-main), response accuracy was analyzed with Generalized Linear Mixed Models (GLMMs).
...
Model selection was based on Akaike Information Criterion[@Grueber2011; @Richards2011].
...
than in the 4 channels ($\beta$ = -2.87, SE = 0.22, *z* (6917) = -13.1, *p* <.001) and
6 channels ($\beta$ = -0.66, SE = 0.19, *z* (6917) = -3.42, *p* < .001).
...
than both high predictability sentences ($\beta$ = 2.18, SE = 0.3, *z* (6917) = 7.2, *p* < .001) and
medium predictability sentences ($\beta$ = -0.52, SE = 0.27, *z* (6917) = -1.97, *p* = .049).
...
at both 6 channels ($\beta$ = 0.33, SE = 0.32, *z* (2590) = 1.04, *p* = .3)
and 8 channels ($\beta$ = -0.014, SE = 0. 32, *z* (2719) = -0.04, *p* = .97).
However, response accuracy was higher in high predictability than in medium predictability sentences
at both 6 channels ($\beta$ = 1.83, SE = 0.65, *z* (2590) = 2.83, p < .005)
and 8 channels ($\beta$ = 1.54, SE = 0.61, *z* (2719) = 2.54, *p* = .011).
