...
These results showed that attention (or lack thereof) can limit predictability effect in degraded speech comprehension: *no attention, no prediction*.
...
which was formalized in Equation \@ref(eq:noisy-channel3) in Chapter \@ref(chapter-introduction) as:

\begin{align*} 
\hat{m_p} &= \argmax_{m_p} P(m_p | u_p) * P(u_p|u_i, N) * P(u_i | m_i) * P(m_i)
\end{align*}

The results of the experiments presented in this thesis as outlined above suggest that listeners are *rational* comprehenders:
...
Instead, our findings are in line with the probabilistic accounts of predictive language processing [@Delong2005; @Kuperberg2016; cf. @Nieuwland2018] proposed in the literature of speech perception and reading studies.
To our knowledge, the empirical evidence presented in this thesis is the only one since @Strauss2013 to examine the nature of predictability effect in degraded speech comprehension.
...
Our findings reported in Chapter \@ref(chapter-attention-prediction) showed that attention and prediction can operate independently (i.e., they are two separate processes) in predictive processing of degraded speech [see also @Astheimer2011; @Li2017].
...
In their *good enough processing* framework, @Ferreira2016 speculated that a comprehender can focus and process one part of sentence "deeper" and other parts can be ignored or processed at a "shallow" level.
...
Contrary to common wisdom and previous findings [e.g., @Dambacher2012; @Wlotko2015], slowing down the speech rate did not improve contextual facilitation [cf. @Winn2021].
...
The speech that the people with sensory neural hearing loss and cochlear implants perceive is spectrally degraded [@Parida2022; @Shannon1998; @Shannon2004].
...
For example, @Pickering2018's account of predictive language processing posits that comprehenders have an access to word-form at the late stage of lexical processing.
...
For example, if 40 out of 50 participants in a cloze norming study may respond that *balloon* is the most likely ending of the truncated sentence *The child went out to fly a red* ___,
...
Similarly,  @Hofmann2021 demonstrated that surprisal-based measures estimated from Large Language Models (e.g., GPT-2, GPT-3, etc.) explain N400 effects and reading times better than cloze-based measures [see @Heilbron2022 for an implementation of surprisal calculated from GPT-2].
...
For example, working memory capacity, processing speed, literacy, and language experience vary across participants and these factors result in the differences among individuals' language processing and the use lexical-semantic cues [@Mishra2012; @Rommers2015; @Scholman2020; @Federmeier2010].
...
For example, it can be speculated that participants with faster processing speed benefit more from the semantic context when the speech rate is fast in Chapter \@ref(chapter-speech-rate) [e.g., @Huettig2016a].
...
One of the goals of the thesis was to replicate the predictability effects in degraded speech comprehension as showed in @Obleser2007 and @Obleser2010.
Therefore, we created short Subject–Verb–Object sentences which were similar to @Obleser2010's stimuli.
...
### The nature of predictability effect

@Strauss2013 formulated their "narrowed expectations" framework based on the results of an EEG study measuring the latency and amplitude of the N400 component.
We did not find a support for their claims;
instead, we demonstrated that the effect of predictability in degraded speech comprehension is *graded* in nature.
...
The research domain of predictive language processing is grapples with the problems like disentangling prediction from integration [e.g., @Mantegna2019] and
parallel vs sequential prediction [see @Gibson2000 for discussion]
